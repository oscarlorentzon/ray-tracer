...
async function generateFrames(
    frames: number,
    frameOffset: number,
    generator: ColorGenerator): Promise<void> {
    const writer: ColorWriter = (frameId, color) => writeFlatSphere({
        width: 128,
        height: 128,
        color,
    ...
    await generateColors(frames, frameOffset, generator, writer);
}

(async function main() {
    const lightnessIncreaser: ColorGenerator =
        (frame, frames) => {
    ...
    const lightnessDecreaser: ColorGenerator =
        (frame, frames) => {
    ...
    const hueRotator: ColorGenerator =
        (frame, frames) => {
