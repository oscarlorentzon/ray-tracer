import { Camera, Coordinates, Point, Vector, } from '../../src/ray-tracer.js';
...
ctx.onmessage = (event) => {
    ...
    if (type === 'render') {
        ...
        resetObjects(event.data.params);
    }
};
function render(params) {
    const size = params.size;
    const camera = new Camera(Math.PI / 3, size.width / size.height);
    camera.lookAt(new Point(ROOM_WIDTH, 5, ROOM_DEPTH), new Point(0, 0, 0), new Vector(0, 1, 0));
    room.populate();
    ...
    for (let y = 0; y < size.height; ++y) {
        ...
        for (let x = 0; x < size.width; ++x) {
            const [viewportX, viewportY] = Coordinates.canvasToViewport(x + 0.5, y + 0.5, size.width, size.height);
...
function resetObjects(contract) {
    room.resetObjects(contract.objects.objects);
}
function log(size, dt, dts) {
    const w = size.width;
    const h = size.height;
